# Upstream usage guide (reviewed derivative)

Read the safety and authorization boundaries in SKILL.md before using any example. All SDK calls are examples, not authorization to run. Example output names must be replaced by new, user-approved output paths. Verify service parameters against official documentation before a paid call.
